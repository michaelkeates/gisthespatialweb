//define DB source
var kenyaexpressways;
var kenyacoords;

function addKenyaExpressway() {
  kenyacoords = { lat: 0.0236, lng: 37.9062 };

  kenyaexpressways = L.tileLayer.wms(
    "https://geoserver.michaelkeates.co.uk/geoserver/geoserver/wms",
    {
      layers: "geoserver:kenya_roads_expressway",
      format: "image/png",
      transparent: true,
      opacity: 1.0,
...
function toggleKenyaExpressway() {
  //get the button id
  var simpleMarker = document.getElementById("toggleKenyaExpresswayBtn");
  ...
  if (mymap.hasLayer(kenyaexpressways)) {
    mymap.removeLayer(kenyaexpressways);
    ...
    mymap.addLayer(kenyaexpressways);
    //pan to the marker
  ...
  //pan to the marker
  mymap.fitBounds(kenyaexpressways.getBounds());
}

//have to set a timer so that this will be rendered after initilizing of the map or it wont appear at all
setTimeout(addKenyaExpressway, 500);
